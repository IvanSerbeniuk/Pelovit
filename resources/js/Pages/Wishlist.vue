...
function loadWishlist() {
    if (window.Wishlist) {
        items.value = window.Wishlist.get()
    }
}
...
function addToCart(item) {
    if (window.Cart) {
        ...
        addedToCart.value[item.id] = true
        setTimeout(() => { addedToCart.value[item.id] = false }, 1500)
    }
}
...
<section class="py-5">
  <div class="container">
    ...
    <p v-if="items.length === 0" class="text-muted">У вас ще немає збережених товарів.</p>
