...
const { addToCart, addedToCart } = useProduct()
    ...
    <div v-else class="row g-4">
      <div v-for="item in wishlistStore.items" :key="item.id" class="col-md-3 col-6">
        <div class="product-card card border-0 shadow-sm rad-16">
          <button class="like active" style="background:none;border:none;cursor:pointer;"
            v-html="HEART_FILLED" @click="wishlistStore.remove(item.id)"></button>
          <NuxtLink :to="`/product/${item.slug}`">
            <img :src="item.image ? '/' + item.image : '/images/image.png'" class="card-img-top" :alt="item.name">
          </NuxtLink>
          <div class="card-body">
            <h6 class="card-title">
              <NuxtLink :to="`/product/${item.slug}`" class="text-decoration-none text-dark">{{ item.name }}</NuxtLink>
            </h6>
            <div class="wrapper__price_buy">
              <h4 class="price">{{ fmt(parseFloat(String(item.price))) }}</h4>
              <button class="btn buy rad-12" @click="addToCart(item)" :disabled="addedToCart[item.id]">
                <span>{{ addedToCart[item.id] ? '✓' : 'Купити' }}</span>
                <span v-if="!addedToCart[item.id]" v-html="CART_SVG"></span>
              </button>
            </div>
          </div>
        </div>
